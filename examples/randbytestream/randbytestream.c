/****************************************************************************
 * randbytestream - Quantum Random Number Generator using IDQ's Quantis Appliance  *
 *                                                                          *
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "ha:s:f:")) != -1) {
        switch (opt) {
            case 'h':
                print_help();
                exit(EXIT_SUCCESS);
...
void print_help(void)
{
    ...
    fprintf(stderr, "%s [-h] [-a domain] [-s no of samples] [-f stream]\n", PROGRAM_NAME);
